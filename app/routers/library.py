...
def _serialize_issue(issue: BookIssue, is_overdue: bool = False) -> dict:
    """Return a plain dict with all ObjectId fields converted to strings."""
    return {
        "id": str(issue.id),
        "college_id": str(issue.college_id),
        ...
        "renewed_count": issue.renewed_count,
        "max_renewals": issue.max_renewals,
        "status": issue.status,
        "fine_amount": issue.fine_amount,
        "fine_paid": issue.fine_paid,
        "fine_paid_date": issue.fine_paid_date.isoformat() if issue.fine_paid_date else None,
        "fine_reason": issue.fine_reason,
        "remarks": issue.remarks,
        "issued_by": issue.issued_by,
        "returned_by": issue.returned_by,
...
async def create_category(
    ...
    cat = BookCategory(college_id=college.id, name=req.name, description=req.description)
    await cat.insert()
    return cat
...
async def update_book(
    ...
    if "category_id" in update_data:
        if update_data["category_id"]:
            ...
            if cat and cat.college_id == college.id:
    ...
    if "total_quantity" in update_data:
        ...
        if new_total < 1:
            raise HTTPException(status_code=400, detail="total_quantity must be at least 1")
        # currently issued = total - available; preserve that count
...
async def delete_book(
    ...
    if not book or book.college_id != college.id:
        raise HTTPException(status_code=404, detail="Book not found")
    active = await BookIssue.find(
        BookIssue.college_id == college.id,
        BookIssue.book_id == book.id,
        BookIssue.status == "issued",
    ).count()
    if active > 0:
        raise HTTPException(status_code=400, detail=f"Cannot delete: {active} active issue(s) exist for this book")
    await book.delete()
...
async def issue_book(
    ...
    if not target_user or target_user.college_id != college.id:
        raise HTTPException(status_code=404, detail="User not found in this college")
    ...
    if req.due_days < 1:
        raise HTTPException(status_code=400, detail="due_days must be at least 1")
...
async def return_book(
    ...
    if issue.status in ("returned", "lost", "damaged"):
        raise HTTPException(status_code=400, detail=f"Book already {issue.status}")
    ...
    if req.is_lost:
        ...
        if now > issue.due_date:
            overdue_days = max(1, (now - issue.due_date).days)
    ...
    # Restore copy only if physically returned (not lost)
    ...
    if fine_amount > 0:
        overdue_days_notify = int((now - issue.due_date).days) if now > issue.due_date else 0
        background_tasks.add_task(
            notify_book_overdue,
            college_id=college.id,
            user_id=str(issue.user_id),
            book_title=issue.book_title,
            overdue_days=overdue_days_notify,
            fine_amount=fine_amount,
            created_by=user.id,
        )

    return {"ok": True, "issue": _serialize_issue(issue), "fine_amount": fine_amount}
...
async def renew_book(
    ...
    if req.additional_days < 1:
        raise HTTPException(status_code=400, detail="additional_days must be at least 1")

    issue.due_date = utcnow() + timedelta(days=req.additional_days)
    issue.renewed_count += 1
    issue.updated_at = utcnow()
    await issue.save()
    return {"ok": True, "issue": _serialize_issue(issue)}
...
async def get_issues(
    ...
    return [_serialize_issue(i, is_overdue=(i.status == "issued" and now > i.due_date)) for i in issues]
...
async def get_my_issues(
    ...
    """Student / faculty — returns their own issues with is_overdue flag."""
...
async def get_library_analytics(
    ...
    if user.role == UserRole.SUPER_ADMIN.value:
        ...
        return {
            ...
            "overdue_issues": len([i for i in all_issues if i.status == "issued" and now > i.due_date]),
            "total_fines_collected": sum(i.fine_amount for i in all_issues if i.fine_paid),
    ...
    return {
        ...
        "total_issued": sum(b.total_quantity - b.available_quantity for b in books),
        "total_categories": len(categories),
        "active_issues": len(active_issues),
        "overdue_issues": len(overdue_issues),
        "returned_today": len([
            i for i in returned_issues
            if i.return_date and i.return_date.date() == now.date()
        ]),
...
# ─── Export (CSV download via fetch, NOT window.open) ──────────────────────────
